(function () {
  ...
  async function loadSubmissionsFromApi() {
    ...
    try {
      var url = new URL('/api/submissions', window.location.origin).href;
      ...
      return { error: res.status, serverError: true };
    } catch (e) {
      return { error: 0, serverError: true };
    }
  }
  ...
  function showScreen(loggedIn) {
    if (loggedIn) {
      ...
      loadSubmissionsFromApi().then(function (list) {
        ...
        } else if (list && list.serverError) {
          currentSubmissions = null;
          renderSubmissions([]);
          if (dashSyncHint) {
            dashSyncHint.style.display = 'block';
            dashSyncHint.textContent = 'Could not load submissions from server. In Vercel: add a Blob store (Storage), set BLOB_READ_WRITE_TOKEN, then redeploy.';
          }
        } else {
    ...
    if (submissionsBody) {
      submissionsBody.innerHTML = list.length
        ? list.map(function (row, i) {
            var businessType = '–';
            if (row.businessOwner === 'yes') {
              if (row.businessType === 'others' && row.businessTypeOther) {
                businessType = 'Others: ' + row.businessTypeOther;
              } else {
